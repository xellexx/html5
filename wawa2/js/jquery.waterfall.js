...
;(function($, window, document){
	$.fn.waterfall = function(options){
		...
		function getJSONData(){
			if(!(isFinish || isLoading)){ // 确保上一次加载完毕才发送新的请求
				...
				//if($(document).innerHeight() - 200 <= $(window).innerHeight() + $(window).scrollTop()){
					// 如果缓存还有数据，直接处理数据
							...
							ajaxFunc(
								function(jsonData){
									...
									try{
										...
										setTimeout(getJSONDataRetry, 5000);
									}
								}, 
								//edit by ljo 2014/01/20
								//function(){
								function(textStatus, xOptions){
									//showMsg('error');
		...
		function dealData(){
			...
			var loopLoad = function() {			
				if (perNum-- > 0 && (data = jsonCache.pop())){
					loadImg([data], opts.imgUrlName, function(){
		...
		return this.each(function(){
			...
			// 增加返回顶部按钮
			$backTop = $('<a class="backToTop"></a>').attr('id', 'backTop').attr('title', '返回顶部').appendTo(document.body);
			...
			$(window).bind('scroll', function(){
				if($(window).scrollTop() > wf_box_top){
					$backTop.stop(true).animate({opacity: 0.6}, 500);
				}else{
	...
	var imgReady = (function(){
		...
		return function (url, ready, load, error) {
			...
			check = function () {
				...
				//50*80毫秒，超时
				check.cnt++ > 80 ? img.onerror(): void(0);				
			};
			check.cnt = 0;
			check.img = img;
			check();
	...
	function loadImg(jsonData, imgUrlName, callback){
			...
			if(data.height >= 0 && data.width >= 0){
				...
				(function(data){
					imgReady(data[imgUrlName], function(width,height){
						...
						data.width = 300;
						data.height = 200;
						data[imgUrlName] = "./images/no-pic.png"
						++count;
					});
				})(data);
			}
		...
		intervalId = setInterval(done, 50);
	}
